...
if(isset($_POST["BtnDate"])){
	...
	$sql = "SELECT * FROM archive WHERE start_date BETWEEN '{$firstDate}' AND '{$lastDate}' ";
...
$active_sheet->setCellValue('G6','Инициатор');
$active_sheet->setCellValue('H6','Дата последнего обновления');
$active_sheet->setCellValue('I6','Автор последнего обновления');
$active_sheet->setCellValue('J6','Время на выполнение');
$active_sheet->setCellValue('K6','Дата завершения');
$active_sheet->setCellValue('L6','Исполнитель');
$active_sheet->setCellValue('M6','Время');
...
case 1:
 $item['department'] = "Отдел кадров";
break;

case 2:
 $item['department'] = "Транспортный отдел";
break;

case 3:
 $item['department'] = "Финансовый отдел";
break;

case 4:
 $item['department'] = "Отдел продаж";
break;

case 5:
 $item['department'] = "Отдел закупок";
break;

case 6:
 $item['department'] = "Отдел маркетинга";
break;

case 7:
 $item['department'] = "Отдел рекламы";
break;

case 8:
 $item['department'] = "Технический отдел";
break;
}
...
$active_sheet->setCellValue('J'.$row_next,$item['deadline']);
$active_sheet->setCellValue('K'.$row_next,$item['finishing']);
$active_sheet->setCellValue('L'.$row_next,$item['performers']);
$active_sheet->setCellValue('M'.$row_next,$item['spent_time']);
			...
			<form method = "post">
				<ul>
					<li>
						...
						<input type="date" id="lastDate" name="firstDate"/>
					</li>
					<li>
				...
				<button type="submit" id="crReport" name="BtnDate" class="Btn">Создать</button>
				</div>
			</form>
		</div>
		<div class="report__item">
		...
		<div class="center-on-page">
		  <div class="select">
			<select name="sitetime" id="sitetime" onchange="document.getElementById('report').value=value">
			  ...
			  <?php 
				$sql = mysqli_query($link, "SELECT DISTINCT performers FROM application")or die("Ошибка");
